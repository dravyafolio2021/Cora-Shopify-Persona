...
function CustomerProfileContent() {
  ...
  const submitCheckin = async (responded: boolean) => {
    ...
    try {
      await axios.post('https://cora-persona-backend.vercel.app/api/checkin/respond', {
        jobId: checkinJobId,
        responded
      });
      // Update local state to reflect the new check-in response immediately
      setCheckins(prev => [
        {
          id: 'temp-' + Date.now(),
          streak_day: responded ? streak + 1 : streak,
          responded: responded,
      ...
      alert('Failed to record check-in response');
    } finally {
      ...
      {showCheckinModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl shadow-xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6">
              <p className="text-sm text-slate-600 text-center mb-6 font-medium leading-relaxed">
                Did you complete your personalized skincare routine for today? Consistency is the key to glowing skin!
              </p>
      ...
      <main className="max-w-4xl mx-auto w-full grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-1 space-y-6">
          ...
          <div className="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-[0_1px_3px_rgba(0,0,0,0.01)]">
            ...
            {/* List all subscribed devices globally */}
            {(() => {
              const subs = Array.isArray(customer?.push_subscription)
                ...
                return (
                  <div className="mt-5 pt-5 border-t border-slate-100">
                    <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-1.5">
                      <Smartphone className="w-3 h-3 text-slate-400" /> Active Devices ({subs.length})
                    </h4>
